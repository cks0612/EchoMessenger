...
-구현 내용과 기능 설명
 ...
 -매소드 .clear를 이용하여 텍스트 박스의 내용을 리스트박스에 입력후 텍스트 박스의 기존 내용이 지워진다.
 
 -엔터 키를 누르면 텍스트 박스의 내용이 리스트 박스로 전송된다.

 -엔터키로 텍스트 박스 내용 입력 후 텍스트 박스에 포커스된다.
 
 -매소드 .focus를 이용하여 전송 버튼을 마우스로 클릭시 자동으로 텍스트 박스에 포커스 된다.
...
-텍스트 박스의 내용 입력시 리스트 박스에서 타임스태프 앞에 추가

-메세지 카운팅, 리스트 박스 아래에 총 메세지 수를 카운팅

-문자열 정제, 메세지의 앞 뒤에 불필요한 공백을 제거한다.
 
 
 -구현내용과기능설명

 -문자열 관련 매소드 .trim을 이용하여 메세지 앞 뒤의 불필여한 공백이 있을 삭제한다.

 -string timeStamp = DateTime.Now.ToString("[HH:mm:ss]"); 코드를 통해 텍스트 박스에서 리스트 박스로 입력 할떄의 시간을 저장한다

 -string message = timeStamp + " " + typed_msg; 코드를 통해 위에서 저장한 시간을 텍스트 박스에서 받은 메세지의 내용 앞에 추가하여 리스트 박스에 입력한다.
